...
type TestFilterFn = (...args: any[]) => any;

describe("Filter: filter", () => {
  let filter: TestFilterFn;
  ...
  it("should take function as predicate", () => {
    ...
    expect(filter(items, (i: { done: any }) => i.done).length).toBe(1);
  });
  ...
  it("should pass the index to a function predicate", () => {
    ...
    const result = filter(
      items,
      (_value: any, index: number) => index % 2 === 0,
    );
  ...
  it("should not be affected by `Object.prototype` when using a string expression", () => {
    (Object.prototype as any).someProp = "oo";
    ...
    delete (Object.prototype as any).someProp;
  });

  it("should throw an error when is not used with an array", () => {
    let item: any = { not: "array" };
  ...
  it("should not throw an error if used with an array like object", () => {
    function getArguments(..._args: any[]) {
      return arguments;
    }
    ...
    function nodeFilterPredicate(node: { innerHTML: string | string[] }) {
  ...
  describe("should support comparator", () => {
    ...
    it("should consider objects with custom `toString()` in non-strict comparison", () => {
      let obj: any = new Date(1970, 1);

      let items: any[] = [{ test: obj }];
    ...
    it("as equality when true", () => {
      let items: any = ["misko", "adam", "adamson"];
    ...
    it("and use the function given to compare values", () => {
      ...
      const comparator = function (obj: any, value: any) {
        return obj > value;
      };
    ...
    it("and use it correctly with deep expression objects", () => {
      ...
      comp = function (actual: unknown, expected: unknown) {
        return (
          isString(actual) &&
          isString(expected) &&
