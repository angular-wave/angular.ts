...
| `policy.check(...)` | Branch on the decision `type`. Common gate policies use `allow`, `deny`, and `redirect`; specialized policies use domain terms such as `deliver`, `drop`, or cache strategies. | Optional reason and metadata explain generic decisions; specialized decisions may add status, target, error, or domain evidence. |
...
| Workflow  | Command timeout and concurrency stay on `WorkflowConfig` or per-run options. Persistence and recovery stay on `WorkflowSupervisorConfig` through `persistence`, `autoPersist`, and `autoRecover`. |
