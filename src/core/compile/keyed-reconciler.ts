/** @internal */
interface KeyedReconciliationEntryBase<T> {
...
/** @internal */
export type KeyedReconciliationEntry<T, TState, TCreated> =
  | (KeyedReconciliationEntryBase<T> & {
...
/** @internal */
export interface KeyedReconciliationPlan<T, TState, TCreated> {
  readonly entries: readonly KeyedReconciliationEntry<T, TState, TCreated>[];
  readonly removed: readonly TState[];
  readonly stable: Uint8Array;
}
 ...
 * @internal
 */
export function planKeyedReconciliation<T, TState, TCreated>(
  items: readonly T[],
  previous: ReadonlyMap<PropertyKey, TState>,
  keyOf: (item: T) => PropertyKey,
  indexOf: (state: TState) => number,
  create: (item: T) => TCreated,
): KeyedReconciliationPlan<T, TState, TCreated> {
  ...
  const entries = new Array<KeyedReconciliationEntry<T, TState, TCreated>>(
    items.length,
  );
  const previousIndexes = new Array<number>(items.length);
  ...
  const removed: TState[] = [];

  for (const [key, state] of previous) {
    if (!seen.has(key)) removed.push(state);
  }
...
/** @internal */
export function findStableKeyedIndexes(
