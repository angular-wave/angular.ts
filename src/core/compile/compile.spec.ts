...
const assert: AnyFn = strictAssert as any;

function asElement<T = Element>(value: any): T {
  return value;
}
...
function isSVGElement(el: any) {
  return !!el.toString().match(/SVG/);
}

function isHTMLElement(el: any) {
  return !!el.toString().match(/HTML/);
}

function getChildScopes(scope: any) {
  let children = [];
 ...
 */
function bootstrap(htmlContent: string, moduleName?: string) {
  dealoc(ELEMENT);
...
describe("$compile", () => {
  ...
  describe("helper functions", () => {
    ...
    it("wraps svg templates in a namespace container", () => {
      ...
      expect(getNodeName(asElement(wrapped[0]))).toBe("circle");
    });
  ...
  function initInjector(name: string) {
  ...
  it("compiles element directives found from several elements", () => {
    ...
    $compile(el);
    expect(getCacheData(asElement(el.childNodes[0]), "idx")).toBe(1);
    expect(getCacheData(asElement(el.childNodes[1]), "idx")).toBe(2);
  });
  ...
  it("compiles element directives from child elements", () => {
    ...
    expect(getCacheData(asElement(el.firstChild), "dir")).toBe(1);
  });
  ...
  it("compiles nested directives", () => {
    ...
    expect(getCacheData(asElement(el.firstChild), "dir")).toBe(2);
    expect(getCacheData(asElement(el.firstChild.firstChild), "dir")).toBe(3);
  });
  ...
  ["data"].forEach((prefix) => {
    ["-"].forEach((delim) => {
      it(`compiles element directives with ${prefix}${delim} prefix`, () => {
        ...
        reloadModules();
        const el = $(
          `<${prefix}${delim}my-directive></${prefix}${delim}my-directive>`,
        );
  ...
  describe("compiler control", () => {
    ...
    describe("template", () => {
      ...
      describe("replace and not exactly one root element", () => {
        ...
        it("should not throw if the root element is accompanied by: whitespace", () => {
          ...
          const element = $compile("<p template></p>")($rootScope);
      ...
      it("should support templates with root <tr> tags", async () => {
        ...
        await wait();
        expect(getNodeName(asElement(ELEMENT.firstChild))).toMatch(/tr/i);
      });
      ...
      it("should support templates with root <optgroup> tags", () => {
        ...
        expect(getNodeName(asElement(ELEMENT.firstChild))).toMatch(/optgroup/i);
      });
      ...
      it("should support SVG templates using directive.templateNamespace=svg", async () => {
        ...
        await wait();
        const child: any = ELEMENT.firstChild.firstChild;

        expect(getNodeName(asElement(child))).toMatch(/a/i);
    ...
    describe("templateUrl", () => {
      ...
      it("should support MathML templates using directive.templateNamespace=math", async () => {
        ...
        expect(getNodeName(asElement(child.firstChild.firstChild))).toMatch(
          /mn/i,
        );
        expect(isUnknownElement(child)).toBe(false);
        expect(isHTMLElement(child)).toBe(false);
  ...
  describe("$compile", () => {
    ...
    describe("transclude", () => {
      describe("content transclusion", () => {
        ...
        it('should not leak if two "element" transclusions are on the same element', async () => {
          ...
          dealoc(asElement(ELEMENT.firstChild));
          await wait();
          expect(Cache.size).toEqual(cacheSize);
        });
    ...
    describe("element transclusion", () => {
      ...
      it("should support transcluded element on root content", async () => {
        ...
        expect(getNodeName(asElement(element.childNodes[1]))).toBe("div");
        $compile(element)($rootScope);
        await wait();
        expect(getNodeName(asElement(element.childNodes[1]))).toBe("#comment");
