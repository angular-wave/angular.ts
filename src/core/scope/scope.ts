...
interface ScopeExpressionDependency {
  readonly _target: object;
  readonly _property: PropertyKey;
}

interface ScopeExpressionObserver {
  readonly _owner: Scope;
  readonly _read: () => unknown;
  readonly _listener: (value: unknown) => void;
  _dependencies: ScopeExpressionDependency[];
  _nextDependencies: ScopeExpressionDependency[];
  _nextPropertiesByTarget: WeakMap<object, Set<PropertyKey>>;
  _scheduled: boolean;
  _disposed: boolean;
...
const scopeExpressionObservers = new WeakMap<
  object,
  Map<PropertyKey, Set<ScopeExpressionObserver>>
>();

let activeScopeExpressionObserver: ScopeExpressionObserver | undefined;

function trackScopeExpressionRead(target: object, property: PropertyKey): void {
  const observer = activeScopeExpressionObserver;

  if (!observer || observer._disposed) return;
...
function unregisterScopeExpressionDependencies(
  observer: ScopeExpressionObserver,
): void {
  for (let index = 0; index < observer._dependencies.length; index++) {
    const dependency = observer._dependencies[index];
    const observersByProperty = scopeExpressionObservers.get(
      dependency._target,
    );
...
function registerScopeExpressionDependencies(
  observer: ScopeExpressionObserver,
): void {
...
function runScopeExpressionObserver(observer: ScopeExpressionObserver): void {
  if (observer._disposed || observer._owner._destroyed) return;
...
function scheduleScopeExpressionObserver(
  observer: ScopeExpressionObserver,
): void {
  if (observer._scheduled || observer._disposed) return;
...
function scheduleScopeExpressionObservers(
  target: object,
  property: PropertyKey,
): void {
  const observersByProperty = scopeExpressionObservers.get(target);

  if (!observersByProperty) return;

  const observers = new Set<ScopeExpressionObserver>();
  const invalidateWholeTarget =
    isArray(target) ||
    isMapTarget(target) ||
    isSetTarget(target) ||
    isDateTarget(target);
  ...
  if (invalidateWholeTarget) {
    for (const propertyObservers of observersByProperty.values()) {
      for (const observer of propertyObservers) observers.add(observer);
    }
  } else {
...
export function observeScopeExpression(
  scope: ng.Scope,
  read: () => unknown,
...
/** @internal Receives whole-model change notifications from scope traps. */
export interface ModelChangeTracker {
  record(property: PropertyKey, target: unknown): void;
}
...
type ArrayMutationWrapper = (...args: unknown[]) => unknown;
...
let defaultParse: ng.ParseService;

let defaultExceptionHandler: ng.ExceptionHandlerService;

/** @internal */
export interface ScopeRuntimeDependencies {
...
function unwrapArrayMutationArgs(args: unknown[]): unknown[] {
...
/** @internal */
export function createRootScopeService(
...
function getSimpleMemberExpression(node: ASTNode): string | undefined {
  if (node._type === ASTType._Identifier) {
    return getNodeName(node);
  }
  ...
  if (!propertyName || (node as ExpressionNode)._computed) {
    return undefined;
  }
...
function getArrayWatchExpressionElement(
  watchProp: string,
  index: number,
): string | undefined {
...
function addMemberExpressionDependency(
  node: ASTNode | undefined,
  listener: Listener,
  keySet: string[],
  seenKeys: Set<string>,
  ...
  const parentKey = resolveWatchKey((node as ExpressionNode)._object);
...
function collectForeignWatchDescriptors(
  node: ASTNode,
  listener: Listener,
  keySet: string[],
  seenKeys: Set<string>,
 ...
 */
export function createScope(): ng.Scope;
...
export function createScope(
  ...
  return getCachedScopeProxy(
    target,
    context ?? new Scope(undefined, undefined, scheduler),
  );
}
...
type NonScopeConstructor = abstract new (...args: never[]) => unknown;
...
function unwrapScopeValue(value: unknown): unknown {
  if (!isProxy(value)) return value;

  return scopeProxyTargets.get(value as object) ?? value._target;
}
...
function getAssignedScopeValue(value: unknown): AssignedScopeValue {
...
function getObjectListenerTarget(value: unknown): object | undefined {
...
function isMapTarget(target: unknown): target is Map<unknown, unknown> {
  try {
...
function isSetTarget(target: unknown): target is Set<unknown> {
  try {
...
function isNativeScopedTarget(
  target: unknown,
): target is Map<unknown, unknown> | Set<unknown> | Date {
...
function unwrapCollectionArgs(args: unknown[]): unknown[] {
  let rawArgs: unknown[] | undefined;
...
function getCachedScopeProxy(target: unknown, handler: Scope): unknown {
...
export class Scope {
  ...
  _setRuntimeDependencies(runtime: ScopeRuntimeDependencies): void {
  ...
  _destroyDisplacedValue(value: unknown, visited = new Set<object>()): void {
    if (!value || typeof value !== "object" || isNonScope(value)) return;
    ...
    if (isProxy(value)) {
      ...
      if (this._children.includes(scopeValue)) {
        if (scopeValue._handler._destroyed) return;
      ...
      const targetValue = scopeValue._target as
    ...
    if (isProxy(oldValue)) {
      ...
      if (isObject(rawValue)) {
        ...
        if (valueChanged) {
          ...
          this._checkListenersForAllKeys(rawValue as ScopeTarget);
      ...
      if (isUndefined(rawValue)) {
        ...
        const oldTarget = oldValue._target as Record<string, unknown>;
      ...
      if (valueChanged) {
        ...
        let expectedTarget: unknown = this._target;
        ...
        if (mayCollectNestedListeners && isObject(rawOldValue)) {
          collectNestedListeners(unwrapScopeValue(oldValue) as ScopeTarget);
        }

        if (mayCollectNestedListeners && isObject(rawValue)) {
          collectNestedListeners(rawValue as ScopeTarget);
        }
  ...
  get(
    ...
    trackScopeExpressionRead(target as object, property);
    ...
    const targetShadowsScopeData =
      isString(property) && !property.startsWith("_") && property in target;
      ...
      const wrapper = (...args: unknown[]) => {
        ...
        try {
          ...
          const result: unknown = Reflect.apply(
            targetProp as (...args: unknown[]) => unknown,
            target,
            rawArgs,
          );
  ...
  _getNativeCollectionMethodWrapper(
    target: Map<unknown, unknown> | Set<unknown> | Date,
    property: PropertyKey,
    ...
    const wrapper = (...args: unknown[]) => {
      ...
      return Reflect.apply(method, target, rawArgs) as unknown;
    };
  ...
  _applyMapMethod(
    ...
    if (property === "set") {
      ...
      const previousValue: unknown = hadKey ? target.get(key) : undefined;
  ...
  _resolveForeignProxyParent(
    ...
    let cursor: unknown = this._foreignProxyTargets.get(rootValue);
  ...
  _recordModelChange(property: PropertyKey, target: unknown): void {
    ...
    switch (type) {
      ...
      case ASTType._MemberExpression: {
        ...
        if (watchProp !== key) {
          ...
          listener._watchParentFn = this._parse(
            getWatchParentExpression(watchProp),
          );
          collectForeignWatchDescriptors(expr, listener, keySet, seenKeys);
      ...
      case ASTType._ArrayExpression: {
        ...
        for (let i = 0, l = elements.length; i < l; i++) {
          ...
          if (registerKey) {
            ...
            const parentKey = resolveWatchKey(
              (element as ExpressionNode)._object,
            );
  ...
  new(childInstance?: ng.Scope): ng.Scope {
    ...
    if (childInstance) {
      const proto: unknown = Object.getPrototypeOf(childInstance);
      ...
      if (proto === Object.prototype || proto === this._target) {
        Object.setPrototypeOf(childInstance as object, this._target as object);
      } else {
        // If child has some other prototype, preserve it but link to this._target
        Object.setPrototypeOf(
          (proto ?? childInstance) as object,
          this._target as object,
        );
      }
  ...
  _registerInheritedKey(key: string): void {
    ...
    while (owner && owner !== this) {
      ...
      owner = (owner.parent as ScopeProxied<ng.Scope> | undefined)?._handler as
        | Scope
        | undefined;
    }
  ...
  static _registerForeignKeyOwner(owner: Scope, key: string): void {
    const ownerValue = (owner._target as Record<string, unknown>)[key];
    ...
    if (broadcast) {
      if (initialChildren) {
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          event = child._handler._eventHelper(
      ...
      if (this.parent) {
        return this.parent._handler._eventHelper(
          { name, event: currentEvent, broadcast },
          ...args,
        );
      } else {
  ...
  destroy(): void;
  /** @internal Destroys this scope, optionally skipping event rebroadcast. */
  // eslint-disable-next-line @typescript-eslint/unified-signatures -- stripInternal removes this overload from the public contract.
  destroy(skipBroadcast: boolean): void;
  destroy(skipBroadcast = false): void {
      ...
      if (isFunction(newVal)) {
        ...
        for (let i = 0, l = newVal.length; i < l; i++) {
          if (isFunction(newVal[i])) {
            newVal[i] = callFunction(
              newVal[i] as RuntimeFunction,
              undefined,
              _originalTarget,
            );
          }
