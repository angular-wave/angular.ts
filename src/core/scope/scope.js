...
export function createScope(target = {}, context) {
  if (!isObject(target) || isNonScope(target) || isProxy(target)) return target;
      ...
      if (isObject(value)) {
        if (hasOwn(target, property)) {
          ...
          for (const k of keyList) {
            if (!nextKeys.has(k)) delete oldValue[k];
          }
        }
  ...
  #notifyListener(listener, target) {
    ...
    try {
      ...
      if (isFunction(newVal)) {
        if (listener.invokeWatchFn) {
          newVal = listener.invokeWatchFn(originalTarget);

          if (isUndefined(newVal)) {
            newVal = listener.invokeWatchFn(target);
          }
        } else {
