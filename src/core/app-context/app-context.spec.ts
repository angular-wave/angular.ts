...
describe("AppContext", () => {
  ...
  it("should validate model restore snapshots and options", () => {
  ...
  it("should report invalid sync restore values, inbound snapshots, and receive disposers", async () => {
    const errors: unknown[] = [];
  ...
  it("should apply the sync failure policy to invalid inbound snapshots", () => {
    const errors: unknown[] = [];
    let applyIgnored!: (snapshot: { value: number }) => void;
    let applyThrown!: (snapshot: { value: number }) => void;
    const model = context.registerModel("counter", () => ({ value: 0 }));
    ...
    applyIgnored([] as never);
    expect(() => applyThrown([] as never)).toThrowError(
      "Model restore snapshot must be a plain object.",
    );
    expect(errors).toEqual([]);
    expect(model.value).toBe(0);
  ...
  it("should validate injectable model sync targets", () => {
    ...
    const emptyInjected = context.registerModel(
      "emptyInjected",
      () => ({ value: 0 }),
