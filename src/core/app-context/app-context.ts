...
export type ModelSyncFailurePolicy = "report" | "throw" | "ignore";

export interface ModelSyncOptions {
  failure?: ModelSyncFailurePolicy;
}
...
function attachModelLifecycle<T extends ModelState>(
  ...
  handler._propertyMap.$restore = (
    ...
    currentOrigin = restoreOptions.origin;

    try {
      model.$batch(() => {
        if (restoreOptions.mode !== "merge") {
  ...
  handler._propertyMap.$sync = (
    ...
    const record: ModelSyncRecord<T> = {
      origin,
      target: syncTarget,
      failure: syncOptions.failure ?? "report",
      disposed: false,
    };
      ...
      try {
        const disposer = syncTarget.receive((incoming, restoreOptions = {}) => {
          ...
          model.$restore(incoming, {
            ...restoreOptions,
            origin: restoreOptions.origin ?? origin,
          });
        });
