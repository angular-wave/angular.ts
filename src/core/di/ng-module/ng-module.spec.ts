...
describe("NgModule", () => {
  ...
  it("applies built-in config during config phase", async () => {
    ...
    expect(routerProvider._defaultSquashPolicy).toBe("~");
  ...
  it("stores high-level injectable helpers as provider factories", () => {
    ngModule
      ...
      .serviceWorker("appServiceWorker", "/sw.js")
  ...
  it("registers named service worker wrappers as provider factories", () => {
    ngModule.serviceWorker("appServiceWorker", "/sw.js", {
      scope: "/",
      type: "module",
    });
    ...
    expect(registration.method).toBe("factory");
    expect(registration.args[0]).toBe("appServiceWorker");
    expect(registration.args[1][0]).toBe(_serviceWorker);
    expect(registration.args[1][1]).toBe(_injector);
    expect(registration.args[1][2]).toBe(_exceptionHandler);
  });

  it("resolves dynamic service worker script and config when invoking generated provider factories", async () => {
    const registration = { scope: "/app/" };
    const resolveScriptUrl = jasmine
      .createSpy("resolveScriptUrl")
      .and.returnValue("/dynamic-sw.js");
    const resolveConfig = jasmine.createSpy("resolveConfig").and.returnValue({
      scope: "/app/",
      updateViaCache: "none",
      autoRegister: false,
    });

    ngModule.serviceWorker("appServiceWorker", resolveScriptUrl, resolveConfig);

    const registrationEntry = ngModule._invokeQueue
      .map(inspectProviderCommand)
      .find(({ args }) => args[0] === "appServiceWorker");
    const factory = registrationEntry.args[1];
    const injectorInvoke = jasmine
      .createSpy("injectorInvoke")
      .and.callFake((value: any) => value());
    const injector = {
    ...
    const baseService = {
      ...
      onControllerChange: jasmine.createSpy("onControllerChange"),
      onUpdate: jasmine.createSpy("onUpdate"),
    };
    const exceptionHandler = jasmine.createSpy("exceptionHandler");

    const service = factory[3](baseService, injector, exceptionHandler);
    ...
    expect(injector.invoke).toHaveBeenCalledWith(resolveScriptUrl);
    expect(injector.invoke).toHaveBeenCalledWith(resolveConfig);
    expect(baseService.register).not.toHaveBeenCalled();

    await service.register();

    expect(baseService.register).toHaveBeenCalledOnceWith("/dynamic-sw.js", {
      scope: "/app/",
      updateViaCache: "none",
      autoRegister: false,
    });
  ...
  it("starts service worker registration only when autoRegister is configured", async () => {
    ...
    const injector = {
      invoke: jasmine.createSpy("injectorInvoke"),
    };
    const exceptionHandler = jasmine.createSpy("exceptionHandler");

    ngModule.serviceWorker("manualServiceWorker", "/manual-sw.js", {
      autoRegister: false,
    });
    ngModule.serviceWorker("autoServiceWorker", "/auto-sw.js", {
      autoRegister: true,
      scope: "/",
    });

    const registrations = ngModule._invokeQueue.map(inspectProviderCommand);
    const manualFactory = registrations.find(
      ({ args }) => args[0] === "manualServiceWorker",
    ).args[1];
    const autoFactory = registrations.find(
      ({ args }) => args[0] === "autoServiceWorker",
    ).args[1];
    const manualBaseService = createBaseService();
    const autoBaseService = createBaseService();

    const manualService = manualFactory[3](
      manualBaseService,
      injector,
      exceptionHandler,
    );
    const autoService = autoFactory[3](
      autoBaseService,
      injector,
      exceptionHandler,
    );

    expect(manualBaseService.register).not.toHaveBeenCalled();
    expect(manualService.status).toBe("idle");
    expect(autoBaseService.register).toHaveBeenCalledOnceWith("/auto-sw.js", {
      autoRegister: true,
      scope: "/",
    });
    expect(autoService.status).toBe("registering");
  ...
  it("lets model sync targets coordinate named machines and workflows", async () => {
    ...
    expect(playerMachine.current).toBe("down");
    expect(playerMachine.data.reason).toBe("health");
    expect(recoveryWorkflow.current).toBe("running");
