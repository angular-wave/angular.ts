...
export interface LogRuntimeDependencies {
  authorizeBeacon?: (context: RequestPolicyContext) => SecurityPolicyDecision;
  now?: () => number;
      ...
      try {
        ...
        if (decision.type !== "allow") {
          if (!reportedSecurityFailure) {
            ...
            reportFailure(
              `security policy denied delivery${decision.reason ? `: ${decision.reason}` : ""}`,
            );
          }
        ...
        if (
          Object.keys(decision.credentials?.headers ?? {}).length > 0 ||
          decision.credentials?.withCredentials === false
        ) {
          if (!reportedSecurityFailure) {
            reportedSecurityFailure = true;
            reportFailure(
              "security policy returned credentials that Beacon cannot apply",
            );
          }
