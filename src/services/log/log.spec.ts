...
describe("$log", () => {
  ...
  it("automatically blocks Beacon delivery that requires Authorization headers", () => {
    const sendBeacon = spyOn(window.navigator, "sendBeacon").and.returnValue(
      true,
    );
    const consoleError = spyOn(window.console, "error");
    ...
    const log = bootstrap("securedBeacon");
    ...
    expect(consoleWarn).toHaveBeenCalledOnceWith(
      "$log Beacon delivery failed: security policy denied delivery: Beacon transport cannot attach Authorization credentials",
    );
  });

  it("gates Beacon delivery before serializing the log entry", () => {
    const configuration = createLogRuntimeConfiguration();
    const consoleError = jasmine.createSpy("consoleError");
    const consoleWarn = jasmine.createSpy("consoleWarn");
    const serializer = jasmine.createSpy("serializer").and.returnValue("body");
    const sendBeacon = jasmine.createSpy("sendBeacon").and.returnValue(true);
    const authorizeBeacon = jasmine
      .createSpy("authorizeBeacon")
      .and.returnValue({ type: "deny", reason: "blocked endpoint" });

    applyLogConfiguration(configuration, {
      beacon: { serializer: "serializer", url: "/logs" },
    });
    createLogService(
      configuration,
      {
    ...
    expect(consoleError).toHaveBeenCalledOnceWith("sensitive");
    expect(authorizeBeacon).toHaveBeenCalledOnceWith({
      operation: "request",
      transport: "beacon",
      method: "POST",
      url: "/logs",
    ...
    expect(sendBeacon).not.toHaveBeenCalled();
    expect(consoleWarn).toHaveBeenCalledOnceWith(
      "$log Beacon delivery failed: security policy denied delivery: blocked endpoint",
    );
  });

  it("reports a Beacon security denial without a reason", () => {
    const configuration = createLogRuntimeConfiguration();
    const consoleWarn = jasmine.createSpy("consoleWarn");
    ...
    createLogService(
      ...
      {
        authorizeBeacon: () => ({ type: "deny" }),
        sendBeacon: () => true,
      },
    ).error("blocked");

    expect(consoleWarn).toHaveBeenCalledOnceWith(
      "$log Beacon delivery failed: security policy denied delivery",
    );
  });

  it("contains synchronous Beacon security failures", () => {
    const configuration = createLogRuntimeConfiguration();
    const consoleWarn = jasmine.createSpy("consoleWarn");
    ...
    expect(consoleWarn).toHaveBeenCalledWith(
      "$log Beacon delivery failed: security policy evaluation threw",
      jasmine.stringContaining("policy failed"),
    );
  });

  it("rejects credentials that a custom Beacon gate cannot apply", () => {
    const configuration = createLogRuntimeConfiguration();
    const consoleWarn = jasmine.createSpy("consoleWarn");
    const sendBeacon = jasmine.createSpy("sendBeacon").and.returnValue(true);
    ...
    expect(sendBeacon).not.toHaveBeenCalled();
    expect(consoleWarn).toHaveBeenCalledOnceWith(
      "$log Beacon delivery failed: security policy returned credentials that Beacon cannot apply",
    );

    const cookieConsoleWarn = jasmine.createSpy("cookieConsoleWarn");
    ...
    ).error("blocked cookie exclusion");

    expect(cookieConsoleWarn).toHaveBeenCalledOnceWith(
      "$log Beacon delivery failed: security policy returned credentials that Beacon cannot apply",
    );
  });
