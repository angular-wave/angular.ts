...
describe("validation helpers", () => {
  ...
  it("throws descriptive TypeErrors for invalid values", () => {
    expect(() => validateIsString(1, "name")).toThrowError(
      TypeError,
    ...
    expect(() => validateArray("x", "items")).toThrowError(
      TypeError,
      'badarg:notarray items="x"',
    );
    expect(() => validateIsNumber("x", "count")).toThrowError(
      TypeError,
      'badarg:fail count="x"',
    );
  });
  ...
  it("falls back to String when invalid values cannot be JSON serialized", () => {
    ...
    expect(() => validateIsString(circular, "value")).toThrowError(
      TypeError,
