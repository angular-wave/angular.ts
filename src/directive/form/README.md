...
This directory owns AngularTS native form and logical `ngForm` group
controller behavior: control registration, aggregate validity, dirty/pristine
state, submitted state, form naming, native submit coordination, and
form-level propagation. The implementation in `form.ts` is centered on
`FormController`, which aggregates child `NgModelController` instances and
logical-group `FormController` instances.
...
- Coordinate native submit/reset for real `<form>` elements while keeping
  logical `ngForm` groups separate from native form behavior.

## Public Surface

- `FormController`: controller class used by `form` and `ngForm`.
- `formDirective`: directive factory for native `<form>` elements.
- `ngFormDirective`: directive factory for logical form groups.
...
- `ControlName`, `PublicValidationState`, `CustomValidationState`,
...
buckets. Child controls are either `NgModelController` instances or
logical-group `FormController` instances. A form can also have a parent form
through `_parentForm`, with `nullFormCtrl` ending the chain.
...
attribute for logical groups.

During compile, the directive adds initial `ng-pristine` and `ng-valid`
classes. During pre-link, it installs native submit/reset listeners only for
real `<form>` elements, registers with the parent form, resolves the scope
setter for the form name, publishes the controller, observes name changes, and
registers destruction cleanup.
...
- `_controls`: ordered child control and logical-group list.
- `_customErrorControls`: map from validation key to controls currently
  invalid for that key.
- `_pendingCustomValidatorControls`: map from validation key to controls with
...
- Native `<form>` reset: lets the browser reset controls, then syncs registered
  model controllers from the resulting DOM values.

## Edge Cases

- `ngForm` can nest logical groups even though native nested `<form>` elements
  are invalid HTML; those groups do not emulate native submit/reset behavior.
...
`FormController`
: Aggregate controller for native forms and logical `ngForm` groups.

`ParentFormController`
: Shared parent-form contract consumed by forms and model controls.

`FormControlController`
: Controller union for registered logical groups and model controls.
...
- `form.spec.ts` covers form registration, submit behavior, logical groups,
  submitted propagation, pristine/dirty state, validity aggregation, named form
  publication, renaming, removal, and CSS classes.
- Changes to validity propagation should test logical groups, removed controls,
