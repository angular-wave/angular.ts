...
export class StateRuntime {
  ...
  _getTransitionFallbackPolicyFromStateName(stateName: StateOrName):
    | {
        state: StateDeclaration;
    ...
    | undefined {
    return (
      this._getStatePolicyFromStateName(
        stateName,
        (state) => state.policy?.transition?.fallbackTo,
      ) ??
      (this._routerState._fallbackTo !== undefined
        ? {
            state: this._stateRegistry._root.self,
            policy: this._routerState._fallbackTo,
          }
        : undefined)
    );
  }
  ...
  async _recoverLazyLoadFailure(
    ...
    const fallbackPolicy = this._getTransitionFallbackPolicyFromStateName(
      toState.name(),
    );

    if (!fallbackPolicy) return undefined;
