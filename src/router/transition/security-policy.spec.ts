...
describe("transition security policy", () => {
  ...
  function bootstrapSecurityTransition(securityConfig = {}, protectedPolicy?) {
    ...
    configureRoutes(app, (routerModule) => {
      ...
      routerModule.router({
        ...
        url: "/protected",
        template: "protected",
        policy: protectedPolicy ? { navigation: protectedPolicy } : undefined,
      });
      routerModule.router({
        name: "login",
        url: "/login",
...
describe("transition configured security policy", () => {
  ...
  it("redirects protected routes through route navigation policy", async () => {
...
describe("transition route-tree navigation policy", () => {
  ...
  function bootstrapRoutePolicy(
    ...
    configureRoutes(app, (routerModule) => {
      ...
      routerModule.router({
        ...
        policy: {
          navigation: {
            permissions: ["admin:audit", "admin:read"],
            redirectTo: "forbidden",
          },
        },
      });
  ...
  it("allows inherited protected routes when authenticated", async () => {
    $state = bootstrapRoutePolicy({
  ...
  it("accepts an array of permissions on the navigation policy", async () => {
    $state = bootstrapRoutePolicy({
      isAuthenticated: true,
      credentials: {
        bearer: () => "token",
    ...
    await goAndAssert("admin.audit", "admin.audit");
  });
...
describe("transition fallback policy", () => {
  ...
  function bootstrapFallbackResolveState(
    ...
    const moduleName = `fallbackTargetModule${moduleId}`;
