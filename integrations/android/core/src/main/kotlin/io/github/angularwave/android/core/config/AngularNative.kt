...
object AngularNative {
    val config: AngularNativeConfig = AngularNativeConfig()

    /**
     * Provides useful version and type information about the system WebView component installed on
     * the device. This can be used in your app to require a minimum system WebView version on the
     * device and point users to the Play Store to update the corresponding app (Google Chrome or
     * Android System WebView).
     */
    fun webViewInfo(context: Context) = WebViewInfo(context.applicationContext)

    /**
     * Loads the [PathConfiguration] JSON file(s) from the provided location to configure navigation
     * rules.
     *
