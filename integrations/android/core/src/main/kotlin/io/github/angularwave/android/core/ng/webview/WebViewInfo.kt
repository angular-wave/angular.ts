...
class WebViewInfo internal constructor(context: Context) {
    ...
    /**
     * The system WebView's major version (corresponds to Chrome or Android System WebView). Returns
     * null if the major version cannot be determined.
     */
    val majorVersion: Int? = packageInfo?.versionName?.substringBefore(".")?.toIntOrNull()

    /**
     * The default User-Agent provided by the system WebView before a call is made to
     * WebView.settings.setUserAgentString(String).
     */
    val defaultUserAgent: String = WebSettings.getDefaultUserAgent(context)

    /**
     * The system WebView's origin type. Different OS versions have the WebView component backed by
     * either Google Chrome or the Android System WebView component. These are updatable through the
     * Play Store.
     */
    val webViewType =
        when {
            packageInfo?.packageName?.contains(PACKAGE_CHROME_WEBVIEW) == true -> WebViewType.CHROME
            packageInfo?.packageName?.contains(PACKAGE_CHROME_PRE_RELEASE_WEBVIEW) == true ->
                WebViewType.CHROME
            packageInfo?.packageName?.contains(PACKAGE_SYSTEM_WEBVIEW) == true ->
                WebViewType.ANDROID_SYSTEM
    ...
    /**
     * The Play Store app Uri for the system WebView type. This is useful to point users to the Play
     * Store if their WebView version is outdated.
     */
    val playStoreWebViewAppUri =
        when (webViewType) {
            WebViewType.ANDROID_SYSTEM -> "market://details?id=${PACKAGE_SYSTEM_WEBVIEW}".toUri()
            WebViewType.CHROME -> "market://details?id=${PACKAGE_CHROME_WEBVIEW}".toUri()
            else -> null
        }
}
