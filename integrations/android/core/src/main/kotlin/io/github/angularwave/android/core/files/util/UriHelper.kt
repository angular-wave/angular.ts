...
import io.github.angularwave.android.core.ng.util.extract
import java.io.File
...
internal class UriHelper(val context: Context) {
    ...
    private fun getContentUriAttributes(
        context: Context,
        uri: Uri,
    ): UriAttributes? {
    ...
    private fun uriAttributesFromContentQuery(
        uri: Uri,
        mimeType: String?,
        cursor: Cursor,
    ): UriAttributes? {
    ...
    private fun uriAttributesDerivedFromUri(
        uri: Uri,
        mimeType: String?,
    ): UriAttributes? {
    ...
    /**
     * Determine if the file points to an app resource. Symbolic link attacks can target app
     * resource files to steal private data.
     */
    ...
    private fun Uri.fileExtension(): String? = lastPathSegment?.extract("\\.([0-9a-z]+)$")

    private fun Uri?.mimeType(): String =
        this?.fileExtension()?.let {
            MimeTypeMap.getSingleton().getMimeTypeFromExtension(it)
        } ?: "application/octet-stream"

    private fun Uri.getFile(): File? = path?.let { File(it) }
}
