...
/**
 * Errors representing WebViewClient.ERROR_* errors received from the WebView when attempting to
 * load a page. https://developer.android.com/reference/android/webkit/WebViewClient
 */
sealed interface WebError : VisitError {
    ...
    companion object {
        fun from(error: WebResourceErrorCompat): WebError {
            val errorCode =
                if (isFeatureSupported(WebViewFeature.WEB_RESOURCE_ERROR_GET_CODE)) {
                    error.errorCode
                } else {
                    0
                }
            ...
            val description =
                if (isFeatureSupported(WebViewFeature.WEB_RESOURCE_ERROR_GET_DESCRIPTION)) {
        ...
        fun from(errorCode: Int): WebError = getError(errorCode, null)
        ...
        private fun getError(
            ...
            WebError::class
                ...
                .firstOrNull { it.errorCode == errorCode } ?: Other(errorCode, description)
    }
}
