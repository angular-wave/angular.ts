...
import io.github.angularwave.android.core.ng.webview.WebViewInfo.WebViewType

class WebViewVersionCompatibility private constructor() {
    companion object {
        /**
         * Determines whether the WebView component version installed on the device is less than the
         * `requiredVersion`.
         *
        ...
        fun isOutdated(
            ...
            val versionInfo = AngularNative.webViewInfo(context)
        ...
        /**
         * Display an alert dialog if the WebView component version installed on the device is less
         * than the `requiredVersion`. The user can tap "Update" to update the corresponding "Google
         * Chrome" or "Android System WebView" app in the Play Store.
         *
         * @return True if the WebView is outdated and the dialog is displayed, otherwise false.
         */
        fun displayUpdateDialogIfOutdated(
            activity: Activity,
            requiredVersion: Int,
        ): Boolean {
            val versionInfo = AngularNative.webViewInfo(activity)

            return if (isOutdated(activity, requiredVersion)) {
                val descriptionResId =
                    when (versionInfo.webViewType) {
                ...
                MaterialAlertDialogBuilder(activity)
                    ...
                    .setNegativeButton(R.string.angular_native_dialog_cancel) { dialog, _ ->
                        dialog.dismiss()
                    }
                    .setPositiveButton(R.string.webview_error_update) { dialog, _ ->
                        try {
                            activity.startActivity(
                                Intent(Intent.ACTION_VIEW, versionInfo.playStoreWebViewAppUri)
                            )
                        } catch (_: ActivityNotFoundException) {
                            Toast.makeText(
                                    activity,
                                    R.string.webview_error_store_unavailable,
                                    Toast.LENGTH_LONG,
                                )
                                .show()
                        }
