...
import io.github.angularwave.android.core.files.delegates.FileChooserDelegate
import io.github.angularwave.android.core.files.delegates.GeolocationPermissionDelegate
import io.github.angularwave.android.core.logging.logEvent
import io.github.angularwave.android.core.logging.logWarning
...
import io.github.angularwave.android.core.ng.errors.WebSslError
import io.github.angularwave.android.core.ng.http.AngularNativeHttpClient
import io.github.angularwave.android.core.ng.http.HttpRepository
import io.github.angularwave.android.core.ng.offline.OfflineHttpRepository
import io.github.angularwave.android.core.ng.offline.OfflinePreCacheRequest
import io.github.angularwave.android.core.ng.offline.OfflineRequestHandler
import io.github.angularwave.android.core.ng.offline.OfflineWebViewRequestInterceptor
import io.github.angularwave.android.core.ng.util.isHttpGetRequest
...
/**
 * This class is primarily responsible for managing an instance of an Android WebView that will be
 ...
 * @property webView An instance of a [AngularNativeWebView] to be shared/managed.
 */
@Suppress("unused")
class Session(
    ...
    /** Provides the status of whether WebNavigation is initialized and ready for use. */
    var isReady = false
        internal set

    /** Indicates whether WebNavigation is currently enabled for this session. */
    fun isWebNavigationEnabled(): Boolean = webNavigationEnabled

    /** Indicates whether WebNavigation/LegacyNavigation is active for this session. */
    private var webNavigationEnabled = false

    /**
     * Specifies whether the render process is gone for the WebView instance. If the render process
     * is gone, this Session and its WebView cannot be reused and must be recreated.
     */
    ...
    init {
        initializeWebView()
        AngularNativeHttpClient.enableCachingWith(context)
    ...
    /**
     * Fetches a given location and returns the response to the [OfflineRequestHandler] Allows an
     * offline cache to contain specific items instead of solely relying on visited items.
     *
     * @param location Location to cache.
     */
    fun preCacheLocation(location: String) {
        val requestHandler =
            checkNotNull(AngularNative.config.offlineRequestHandler) {
                "An offline request handler must be provided to pre-cache $location"
            }

        offlineHttpRepository.preCache(
            requestHandler,
            OfflinePreCacheRequest(
                url = location,
                userAgent = AngularNative.config.userAgentWithWebViewDefault(context),
            ),
        )
    }
    ...
    fun visit(visit: Visit) {
        ...
        when {
            visit.options.action == VisitAction.RESTORE &&
                !isColdBooting &&
                restoreVisitFromHistory(visit) -> {}

            isColdBooting -> {
                visitPending = true
            }

            isReady && webNavigationEnabled -> {
                visitLocation(visit)
            }
    ...
    /**
     * Synthetically restore the WebView's current visit without using a cached snapshot or a visit
     * request. This is used when restoring a destination from the backstack, but the WebView's
     * current location hasn't changed from the destination's location.
     */
    ...
    // Callbacks from WebNavigation JS Core

    /**
     * Called by WebNavigation bridge when a new visit is proposed.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
     * @param location The location to visit.
     * @param optionsJson A JSON block to be serialized into [VisitOptions].
     */
    @JavascriptInterface
    fun visitProposedToLocation(
        location: String,
        optionsJson: String,
    ) {
    ...
    /**
     * Called by WebNavigation bridge when a new visit proposal will refresh the current page.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
     ...
     */
    @JavascriptInterface
    fun visitProposalRefreshingPage(
        location: String,
        optionsJson: String,
    ) {
        val options = VisitOptions.fromJSON(optionsJson) ?: return
    ...
    /**
     * Called by WebNavigation bridge when a new visit proposal will scroll to an anchor on the same
     * page.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     ...
     */
    @JavascriptInterface
    fun visitProposalScrollingToAnchor(
        location: String,
        optionsJson: String,
    ) {
        val options = VisitOptions.fromJSON(optionsJson) ?: return
    ...
    /**
     * Called by WebNavigation bridge when a new visit has just started.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called by WebNavigation bridge when the HTTP request has started.
     *
     * @param visitIdentifier A unique identifier for the visit.
     */
    @JavascriptInterface
    ...
    /**
     * Called by WebNavigation bridge when the HTTP request has been completed.
     *
     * @param visitIdentifier A unique identifier for the visit.
     */
    @JavascriptInterface
    ...
    /**
     * Called by WebNavigation bridge when the HTTP request has failed.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called by WebNavigation bridge when a visit request fails with a non-HTTP status code,
     * suggesting it may be the result of a cross-origin redirect visit. Determining a cross-origin
     * redirect is not possible in javascript with the Fetch API due to CORS restrictions, so verify
     * on the native side. Propose a cross-origin redirect visit if a redirect is found, otherwise
     * fail the visit.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
    ...
    /**
     * Called by WebNavigation bridge once the page has been fully loaded by the WebView.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called by WebNavigation bridge once the page has been fully rendered in the webView.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called by WebNavigation bridge when the visit is fully completed (request successful and page
     * rendered).
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
    ...
    /**
     * Called by WebNavigation bridge when a form submission has started.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called by WebNavigation bridge when a form submission has finished.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
    ...
    /**
     * Called when WebNavigation bridge detects that the page being visited has been invalidated,
    ...
    /**
     * Sets internal flags that indicate whether WebNavigation in the WebView is ready for use.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     *
     ...
     */
    @JavascriptInterface
    fun webNavigationIsReady(isReady: Boolean) {
        logEvent("webNavigationIsReady", "isReady" to isReady)
        ...
        currentVisit?.let { visit ->
            ...
            if (!isReady) {
                ...
                logEvent("webNavigationIsNotReady", "error" to visitError)
    ...
    /**
     * Sets internal flags indicating that WebNavigation did not properly initialize.
     *
     * Warning: This method is public so it can be used as a Javascript Interface. You should never
     * call this directly as it could lead to unintended behavior.
     */
    @JavascriptInterface
    fun webNavigationFailedToLoad() {
        val visitError = LoadError.NotPresent

        logEvent("webNavigationFailedToLoad", "error" to visitError)
        reset()
        callback { it.onReceivedError(visitError) }
    }

    /**
     * Sets whether this session should use WebNavigation/LegacyNavigation navigation.
     *
     * Some pages intentionally omit AngularNative, and still need full page navigation with native
     * shell handoff.
     *
     * @param isWebNavigationEnabled true when a WebNavigation bridge is present.
     */
    @JavascriptInterface
    fun setWebNavigationEnabled(isWebNavigationEnabled: Boolean) {
        logEvent("setWebNavigationEnabled", "isWebNavigationEnabled" to isWebNavigationEnabled)
        this.webNavigationEnabled = isWebNavigationEnabled
    }
    ...
    private fun visitLocation(visit: Visit) {
        val restorationIdentifier =
            if (visit.options.action == VisitAction.RESTORE) {
        ...
        val options =
            if (visit.options.action == VisitAction.RESTORE && restorationIdentifier == null) {
    ...
    private fun visitLocationAsColdBoot(visit: Visit) {
        ...
        // When a page is invalidated by WebNavigation, we need to reload the
    ...
    private fun restoreVisitFromHistory(visit: Visit): Boolean {
        val backForwardList =
            runCatching { webView.copyBackForwardList() }.getOrNull() ?: return false
    ...
    internal fun restoreCurrentHistoryVisit(useCurrentVisitIdentifier: Boolean = false) {
        val restorationIdentifier =
            currentVisit?.let { currentVisit ->
                restorationIdentifiers[currentVisit.destinationIdentifier] ?: ""
            } ?: ""

        val visitIdentifier =
            currentVisit?.identifier?.ifEmpty {
                when {
    ...
    private fun String.historyIdentifier(): String = hashCode().toString()
    ...
    private fun String.normalizedLocation(): String =
        try {
            ...
            val port =
                uri.port.takeIf { it != -1 && it != defaultPortFor(scheme) }?.let { ":$it" } ?: ""
            val path =
                uri.path
                    .orEmpty()
                    .ifBlank { "/" }
    ...
    private fun defaultPortFor(scheme: String): Int {
        val httpDefaultPort = 80
        val httpsDefaultPort = 443
    ...
    /**
     * Updates to the DOM are processed asynchronously, so the changes may not immediately be
     * reflected visually by subsequent WebView.onDraw invocations. Use a VisualStateCallback to be
     * notified when the contents of the DOM are ready to be drawn.
     */
    private fun postVisitVisualStateCallback(visitIdentifier: String) {
        if (!isFeatureSupported(VISUAL_STATE_CALLBACK)) return

        context.runOnUiThread {
            WebViewCompat.postVisualStateCallback(webView, visitIdentifier.toRequestId()) {
                requestId ->
    ...
    private fun initializeWebView() {
        val webViewInfo = AngularNative.webViewInfo(context)
        ...
        webView.apply {
            addJavascriptInterface(this@Session, "WebSession")
            webChromeClient = WebChromeClient()
            webViewClient = AngularNativeWebViewClient()
    ...
    internal fun installBridge(
        location: String,
        onBridgeInstalled: () -> Unit = {},
    ) {
    ...
    private fun String.toRequestId(): Long = hashCode().toLong()

    internal fun callback(action: (SessionCallback) -> Unit) {
        context.runOnUiThread {
    ...
    internal fun handleRenderProcessGone(view: WebView): Boolean {
        if (view == webView) {
    ...
    // The override below handles renderer termination; AndroidX lint does not resolve it here.
    @SuppressLint("MissingOnRenderProcessGone")
    private inner class AngularNativeWebViewClient : WebViewClientCompat() {
        ...
        override fun onPageFinished(
            ...
            val shouldHandleColdBootCompletion =
                isColdBooting || isHistoryRestoring || !webNavigationEnabled
        ...
        private fun isAtInitialScale(scale: Float): Boolean = initialScale == scale

        /**
         * WebNavigation will not call adapter.visitProposedToLocation in some cases, like
         * target=_blank or when the domain doesn't match. We still route those here. This is only
         * called when links within a webView are clicked and during a redirect while cold booting.
        ...
        override fun shouldInterceptRequest(
            view: WebView,
            request: WebResourceRequest,
        ): WebResourceResponse? = offlineRequestInterceptor.interceptRequest(request)
        ...
        /**
         * Prevents firing twice in a row within a few milliseconds of each other, which happens
         * sometimes. So we check for a slight delay between requests, which is plenty of time to
         * allow for a user to click the same link again.
         */
        ...
        private fun String.identifier(): String = hashCode().toString()
    }
}
