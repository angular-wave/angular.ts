...
internal class OfflineWebViewRequestInterceptor(val session: Session) {
    private val offlineRequestHandler
        get() = AngularNative.config.offlineRequestHandler

    private val httpRepository
        get() = session.offlineHttpRepository

    private val currentVisit
        get() = session.currentVisit
    ...
    fun interceptRequest(request: WebResourceRequest): WebResourceResponse? {
        ...
        return if (isCurrentVisitRequest) {
            ...
            // lets the WebView handle the request/response and WebNavigation can see the redirect,
    ...
    private fun logCurrentVisitResult(
        url: String,
        result: OfflineHttpRepository.Result,
    ) {
        logEvent(
            "location" to url,
