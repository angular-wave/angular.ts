...
sealed interface HttpError : VisitError {
    ...
    companion object {
        ...
        fun from(errorResponse: WebResourceResponse): HttpError =
            getError(errorResponse.statusCode, errorResponse.reasonPhrase)

        fun from(statusCode: Int): HttpError = getError(statusCode, null)

        private fun getError(
            statusCode: Int,
            reasonPhrase: String?,
        ): HttpError {
            if (statusCode in CLIENT_ERROR_CODES) {
            ...
            if (statusCode in SERVER_ERROR_CODES) {
