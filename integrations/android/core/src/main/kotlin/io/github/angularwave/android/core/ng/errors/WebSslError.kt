...
/**
 * Errors representing SslError.SSL_* errors received from the WebView when attempting to load a
 * page. https://developer.android.com/reference/android/net/http/SslError
 */
sealed interface WebSslError : VisitError {
    ...
    companion object {
        fun from(error: SslError): WebSslError = getError(error.primaryError)

        fun from(errorCode: Int): WebSslError = getError(errorCode)

        private fun getError(errorCode: Int): WebSslError =
            WebSslError::class
                .sealedSubclasses
                .mapNotNull { it.objectInstance }
                .firstOrNull { it.errorCode == errorCode } ?: Other(errorCode)
    }
}
