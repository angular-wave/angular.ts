// Reference-only AngularNative/Turbo native adapter. Runtime now uses no-turbo-bridge.js.
(() => {
  ...
  const setupOnLoad = () => {
    ...
    if (!window.Turbo && !window.Turbolinks) {
      // Experiment mode: some pages intentionally do not ship with AngularNative.
