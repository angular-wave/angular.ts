(() => {
  const PENDING_VISIT_KEY = "angular-native-fallback-visit";
  ...
  class NativeNavigation {
    ...
    registerAdapter() {
      if (window.WebSession) {
        WebSession.setWebNavigationEnabled(false);
        WebSession.webNavigationIsReady(true);
      }
    }
    ...
    pageLoaded() {
      ...
      this.afterNextRepaint(function () {
        if (window.WebSession) {
          WebSession.pageLoaded(restorationIdentifier);
        }

        window.nativeNavigation.finishPendingNavigation();
      });
    }

    pageLoadFailed() {
      if (window.WebSession) {
        WebSession.webNavigationFailedToLoad();
      }
    }
    ...
    visitLocationWithOptionsAndRestorationIdentifier(location, optionsJSON) {
      ...
      if (!window.WebSession) return;

      const visitIdentifier = this.nextFallbackVisitIdentifier();
      WebSession.visitStarted(
        visitIdentifier,
        false,
        false,
        normalizedLocation,
      );
      WebSession.visitRequestStarted(visitIdentifier);
    ...
    finishPendingNavigation() {
      ...
      WebSession.visitRequestCompleted(pendingVisit.identifier);
      WebSession.visitRequestFinished(pendingVisit.identifier);

      this.afterNextRepaint(function () {
        WebSession.visitRendered(pendingVisit.identifier);
        WebSession.visitCompleted(pendingVisit.identifier, "");
      });
    }
    ...
    visitRenderedForColdBoot(visitIdentifier) {
      this.afterNextRepaint(function () {
        if (window.WebSession) {
          WebSession.visitRendered(visitIdentifier);
        }
      });
    }
  ...
  const elementTouchStart = (event) => {
    ...
    while (element) {
      ...
      if (preventPullToRefresh) {
        if (window.WebSession) {
          WebSession.elementTouchStarted(true);
        }
        break;
      }
  ...
  const elementTouchEnd = () => {
    if (window.WebSession) {
      WebSession.elementTouchEnded();
    }
  };

  if (window.nativeNavigation == null) {
    window.nativeNavigation = new NativeNavigation();
    window.nativeNavigation.registerAdapter();
    window.nativeNavigation.pageLoaded();
  }
