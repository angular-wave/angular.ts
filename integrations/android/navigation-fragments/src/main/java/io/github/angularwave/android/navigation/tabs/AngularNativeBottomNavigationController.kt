...
/**
 * A [BottomNavigationView] controller that manages multiple [AngularNativeBottomTab]s, each
 * associated with its own [NavigatorHost] instance in the Activity layout.
 */
class AngularNativeBottomNavigationController(
    ...
    enum class Visibility {
        /**
         * Visible by default, but hidden in modal screens and when the virtual keyboard is present
         * on screen.
         */
        DEFAULT,
    ...
    /**
     * Routes to the specified location in the current tab. The resulting destination and its
     * presentation will be determined using the path configuration rules.
     *
    ...
    private fun updateVisibility() {
        ...
        if (visible != view.isVisible) {
            if (animateVisibilityChanges) {
                view.translationYAnimator(VISIBILITY_ANIMATION_DURATION_MS, visible).start()
            } else {
    ...
    private val AngularNativeBottomTab.navigatorHost: NavigatorHost
        get() {
            val fragment =
                activity.supportFragmentManager.findFragmentById(configuration.navigatorHostId)
