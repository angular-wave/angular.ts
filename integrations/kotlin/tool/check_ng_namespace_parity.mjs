...
if (missing.length > 0) {
  console.error("Missing Kotlin parity entries:");
  for (const typeName of missing) console.error(`- ${typeName}`);
}

if (stale.length > 0) {
  console.error("Stale Kotlin parity entries:");
  for (const typeName of stale) console.error(`- ${typeName}`);
}

if (duplicates.length > 0) {
  console.error("Duplicated Kotlin parity entries:");
  for (const typeName of new Set(duplicates)) console.error(`- ${typeName}`);
}

if (invalidStatuses.length > 0) {
  console.error("Invalid Kotlin parity statuses:");
...
console.log(
  `ng namespace parity checklist covers ${sourceTypes.length} public types.`,
);
