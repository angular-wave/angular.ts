...
test("C++ Wasm todo demo updates AngularTS scope through WasmScope", async ({
  ...
  page.on("request", (request) => {
    ...
    if (url.includes("/integrations/wasm/cpp/examples/todo/main.wasm")) {
  ...
  await expect(page.locator("#cpp-remaining")).toContainText("2 of 2");
  ...
  await expect(page.locator("#cpp-remaining")).toContainText("3 of 3");
  await expect(input).toHaveValue("");

  await rows.first().getByRole("checkbox").check();
  await expect(rows.first().getByRole("checkbox")).toBeChecked();
  await expect(rows.first()).toHaveClass(/done/);
  await expect(page.locator("#cpp-remaining")).toContainText("2 of 3");
