...
const parityPath = resolve(
  root,
  "integrations/wasm/go/NG_NAMESPACE_PARITY.md",
);
...
if (missing.length || stale.length || invalidStatuses.length) {
  if (missing.length) {
    console.error("Missing Go parity entries:");
    for (const name of missing) console.error(`- ${name}`);
  }

  if (stale.length) {
    console.error("Stale Go parity entries:");
    for (const name of stale) console.error(`- ${name}`);
  }

  if (invalidStatuses.length) {
    console.error("Invalid Go parity statuses:");
...
console.log(`Go namespace parity covers ${namespaceTypes.size} ng types.`);
