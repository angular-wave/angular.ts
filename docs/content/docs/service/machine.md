...
const game = $machine({
  ...
  data: { score: 0 },
  states: {
    ready: { on: { start: { to: 'playing' } } },
    playing: { on: { tap: { to: 'won' } } },
    won: { on: { restart: { to: 'playing' } } },
  },
});
```
...
`to(context)` resolver handles transitions whose target depends on runtime data.
...
```js
app.factory('sessionWorkflowMachine', [
  '$machine',
  function ($machine) {
    return $machine({
      initial: 'idle',
...
synchronous. Put async authorization, retries, persistence, and recovery in
`$workflow` or another service and then project the result back through a
machine event.
...
const game = $machine({
  initial: 'playing',
  data: {board: Array(9).fill('-'), nextPlayer: 'X'},
  states: {
    playing: {on: {move: {
      guard: ({data, payload}) => data.board[payload.index] === '-',
      update({data, payload}) {
        data.board[payload.index] = data.nextPlayer;
        data.nextPlayer = data.nextPlayer === 'X' ? 'O' : 'X';
      },
      to: ({data, from}) => winnerOf(data.board) || from,
    }}},
    xWon: {},
    oWon: {},
    draw: {},
...
```html
<button
  ng-repeat="cell in $ctrl.game.data.board"
...
whose turn is next. An invalid move records `lastError` in `denied`, leaving
`state` unchanged. Machine transitions do not roll back data mutations when
hooks throw.

### Persist With Transition Hooks

Transition hooks are a convenient place to persist the game after each handled
move. The hook runs after `state` has been updated, so the snapshot contains the
terminal state when the move ends the game.

```js
const persistedGame = $machine({
  ...gameConfig,
  hooks: {
    transition: ({ machine }) =>
      localStorage.setItem('game', JSON.stringify(machine.snapshot())),
  },
});

const saved = localStorage.getItem('game');
if (saved) persistedGame.restore(JSON.parse(saved));
```
...
exist in `states`. Invalid built-in machine states fail at their boundary
instead of leaving the machine in an unreachable state.
...
const session = $machine({
  ...
  data: { status: 'idle' },
  states: {
    setup: { on: { join: { to: 'waiting' } } },
    waiting: { on: { matched: { to: 'playing' } } },
    playing: {},
  },
  hooks: {
    exit: { setup: ({ data }) => (data.status = 'joining') },
    enter: { waiting: ({ data }) => (data.status = 'waiting') },
    transition: ({ type, from, to }) => console.log(type, from, to),
  },
});
```
...
prior data/state mutations are kept, and AngularTS still schedules bound scopes
for the keys touched by the attempted transition.
  ...
  states: {
    setup: {
      on: {
        join: {
          to: 'waiting',
          update: ({ data, payload }) => (data.roomId = payload.roomId),
        },
      },
    },
    waiting: { on: { reset: { to: 'setup' } } },
  },
});
```
