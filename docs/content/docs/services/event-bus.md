...
Configure one callable policy when event delivery must follow an application-wide
rule. The callback is contextually typed as `EventDeliveryPolicy`, so ordinary
configuration does not require a separate decision type.
