...
AngularTS enhances native HTML forms with a form controller that tracks validity
...
the current scope. `ng-form` does the same but can be used on non-form elements
or for nested forms.
...
The `name` attribute (here `registrationForm`) is the scope property under which
...
Map of validator names to arrays of controls that are failing that validator.
E.g., `{ required: [ctrl1], email: [ctrl2] }`.
...
Resets the form and all controls to pristine state. Useful after a successful
save.
...
Apply validators as attributes on `<input>`, `<textarea>`, or `<select>`
elements. When validation fails, the validator's name appears in `error`.

#### `required`

- **Type:** `none`

The field must have a non-empty value. Alias: `ng-required="expression"` for
...
HTML input type. AngularTS adds validators for `email`, `url`, `number`, `date`,
`time`, `week`, `month`, `datetime-local`.
...
By default, only the first matching message is shown. Add this attribute to show
all matching messages simultaneously.
...
The validator key to match against the `error` object. Shown when `error[key]`
is truthy.
