...
function drill(c) {
  if (!c.child)
    throw new Error(
      "'drill' called on an unmounted Component. This should never happen, please make an issue.",
    );
  switch (c.child.type) {
    case 0 /* VComp */:
...
function diff(c, n, parent, context) {
  if (!c && !n) return;
  else if (!c) create(n, parent, context);
  else if (!n) destroy(c, parent, context);
  ...
  } else replace(c, n, parent, context);
}
...
function callDestroyed(c) {
  if (c.type === 1 /* VNode */ && c.onDestroyed) c.onDestroyed();
  if (c.type === 0 /* VComp */) unmountComponent(c);
}
...
function callBeforeDestroyedRecursive(c) {
  ...
  switch (c.type) {
    case 1 /* VNode */:
      for (const child of c.children) {
        if (child.type === 2 /* VText */) continue;
...
function populateDomRef(c, context) {
  if (c.ns === "svg") {
    ...
    c.domRef = context.createElementNS(
...
function drawCanvas(c) {
  if (c.tag === "canvas" && c.draw) c.draw(c.domRef);
}
  ...
  if (!stack.length) {
    ...
    if (obj.type === 2 /* VText */) {
      ...
      if (!obj.child) {
        if (debug) {
          console.error(
            "VComp has no child property set during event delegation",
            obj,
          );
          console.error(
            "This means the Component has not been fully mounted, this should never happen",
          );
          throw new Error(
            "VComp has no .child property set during event delegation",
          );
...
var delegatedEvents = [{ name: "click", capture: true }];
var m = (tag, obj) => {
  ...
  m("div", {
    ...
    children: [
      m("button", {
        ...
        events: {
          captures: { click: eventAction(dispatch2(op, op)) },
          bubbles: {},
        },
        children: [vtext(msg)],
      }),
...
var viewTable = () => {
  return m("table", {
    ...
    children: [
      m("tbody", { props: { id: "tbody" }, children: model.data.map(makeRow) }),
    ],
  });
};
var makeRow = (x) => {
  return m("tr", {
    key: x.id,
    events: {
      captures: { click: eventAction(dispatch2("select", x.id)) },
      bubbles: {},
    },
          ...
          m("a", {
            ...
            children: [
              m("span", {
                ...
                events: {
                  captures: {
                    click: eventActionStop(dispatch2("delete", x.id)),
                  },
                  bubbles: {},
                },
