...
function bufferEntries(observable2, suspense) {
  if (suspense) {
    return combineLatest([suspense, observable2]).pipe(
...
function bindElementBinds(
  element,
  description,
  { complete, error, suspense, subscription },
) {
  const schedulables = [];
  const binds = [
    ...Object.entries(description.bind).map(([key, observable2]) => [
      key,
      observable2,
      false,
    ]),
    ...Object.entries(description.immediateBind).map(([key, observable2]) => [
      key,
      observable2,
      true,
    ]),
  ...
  for (const [key, observable2, immediate] of binds) {
    if (schedulable(key, immediate)) {
      ...
      subscription.add(
        bindObjectKey(element, key, observable2, error, complete),
      );
    }
  }
  if (schedulables.length) {
    const scheduled2 = schedulables.map(([key, observable2]) =>
      makeEntries(key, observable2),
    );
    subscription.add(
      bindObjectChanges(
        element,
        bufferEntries(merge(...scheduled2), suspense),
        error,
        complete,
      ),
    );
...
function bindElementChildren(
  ...
  if (description.childrenBind) {
    if (description.childrenBindMode === "replace") {
      const placeholder = document2.createComment(
        `replaceable child component`,
      );
      element.append(placeholder);
      const activeChild = description.childrenBind.pipe(
        switchMap((child) =>
          componentWirer(child, context2, void 0, document2),
        ),
      );
      ...
      subscription.add(
        description.childrenBind.subscribe({
          next(child) {
            ...
            subscription.add(
              componentRunner(element, child, context2, placeholder, document2),
            );
          },
          error,
          complete: () => {
...
function bindElementClasses(
  element,
  description,
  { complete, error, subscription, suspense },
) {
  ...
  for (const [key, observable2] of Object.entries(
    description.immediateClassBind,
  )) {
    subscription.add(
      bindClassListKey(element, key, observable2, error, complete),
    );
  }
}
function bindElementStyles(
  ...
  for (const [key, observable2] of Object.entries(
    ...
    subscription.add(
      bindObjectKey(element.style, key, observable2, error, complete),
    );
  }
}
...
function wireSuspense(description, context2, document2 = globalThis.document) {
  ...
  const suspense = context2.suspense
    ? combineLatest([props.when, context2.suspense]).pipe(
        map(([a, b]) => a || b),
      )
    : props.when;
  ...
  if (props.suspenseView) {
    ...
    return combineLatest([props.when, main2, suspenseView]).pipe(
      map(([suspend, main3, suspenseView2]) =>
        suspend ? suspenseView2 : main3,
      ),
...
function wireInternal(
  ...
  const componentContext = {
    bindEffect(observable2, effect) {
      context2.isStaticComponent = false;
      subscription.add(
        observable2.pipe(observeOn(animationFrameScheduler)).subscribe({
          next(value) {
  ...
  try {
    ...
    for (const [element, bindDescription] of elementBinds) {
      subscription.add(
        bindElement(element, bindDescription, bindContext, document2),
      );
    }
...
function wire(
  ...
  return new Observable((subscriber) =>
    wireInternal(description, subscriber, context2, outerContainer, document2),
  );
}
function runInternal(
  ...
  const observable2 = isObservableComponent
    ? component
    : wire(component, context2, container, document2);
...
function selectBindings(container, description) {
  ...
  const elementBinds = elementSelectors.map(([selector, desc]) => [
...
function buildElement(description, context2, document2 = globalThis.document) {
  ...
  if (description.element.includes(":")) {
    ...
    element = document2.createElementNS(
      context2.defaultNamespace,
      description.element,
    );
  } else {
...
function buildNode(
  ...
  switch (description.type) {
    ...
    case "component": {
      const componentComment = document2.createComment(
    ...
    case "fragment":
      if (
        ...
        const fragmentComment = document2.createComment(
...
var stampOrBuildStrategy =
  (stamps2) => (component, properties, context2, container, document2) => {
    ...
    if (stamp) {
      ...
      if (
        container2.nodeType === container2.DOCUMENT_FRAGMENT_NODE &&
...
function Row({ vm }, { bindEffect, bindImmediateEffect, events }) {
  ...
  bindEffect(vm.app.rowsToUpdate.pipe(filter((id2) => id2 === vm.id)), () =>
    vm.updateLabel(),
  );
  ...
  return /* @__PURE__ */ jsx(
    ...
    /* @__PURE__ */ jsx(
      ...
      { class: "col-md-4" },
      /* @__PURE__ */ jsx("a", {
        bind: { innerText: vm.label },
        events: { click: events.select },
      }),
    ),
    /* @__PURE__ */ jsx(
      "td",
      ...
      /* @__PURE__ */ jsx(
        "a",
        { type: "button", events: { click: events.remove } },
        /* @__PURE__ */ jsx("span", {
...
function App(_props, { bindEffect, bindImmediateEffect, events }) {
  ...
  const children = vm.rows.pipe(
    map((row) => () => /* @__PURE__ */ jsx(Row, { vm: row })),
  );
    ...
    /* @__PURE__ */ jsx(
      ...
      /* @__PURE__ */ jsx(
        ...
        /* @__PURE__ */ jsx(
          "div",
          { class: "col-md-6" },
          /* @__PURE__ */ jsx("h1", null, "Butterfloat"),
        ),
        /* @__PURE__ */ jsx(
          "div",
          { class: "col-md-6" },
