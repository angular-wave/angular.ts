...
async function runCPUBenchmark(framework, benchmark, benchmarkOptions) {
    ...
    try {
        for (let i = 0; i < benchmarkOptions.batchSize; i++) {
            ...
            let trace = { traceEvents: [] }; //await fs.open(fileNameTrace(framework, benchmark.benchmarkInfo, i), "w");
            ...
            let p = new Promise((resolve) => {
                cdpConnection._wsConnection.on("message", async (msg) => {
...
export async function executeBenchmark(framework, benchmarkId, benchmarkOptions) {
    let runBenchmarks = benchmarks.filter((b) => benchmarkId === b.benchmarkInfo.id && b instanceof CPUBenchmarkWebdriverCDP);
...
//# sourceMappingURL=forkedBenchmarkRunnerWebdriverCDP.js.map