...
`;
function isKeyedRun(result, shouldBeKeyed) {
...
function isKeyedRemove(result, shouldBeKeyed) {
...
function isKeyedSwapRow(result, shouldBeKeyed) {
...
async function assertChildNodes(elem, expectedNodes, message) {
    let elements = await elem.$$("*");
    let allNodes = await Promise.all(elements.map((e) => e.evaluate((e) => e.tagName)));
    let toLower = (array) => array.map((s) => s.toLowerCase());
...
function niceEmptyString(val) {
    if (!val || val.length === 0)
        return "[empty]";
    if (val.every((v) => v.length === 0))
        return "[empty]";
    return val.toString();
}
async function assertClassesContained(elem, expectedClassNames, message) {
    ...
    if (!expectedClassNames.every((expected) => actualClassNames.includes(expected))) {
        console.log("css class not correct. Expected for " +
            message +
            " to be " +
            expectedClassNames +
            " but was " +
...
export async function checkTRcorrect(page) {
    ...
    if (!span || !(await assertClassesContained(span, ["glyphicon", "glyphicon-remove"], "span in a in third td"))) {
        return false;
    }
...
async function runBench(frameworkNames // Not used in the function, but is used when calling the function in other files
) {
    var _a;
    let runFrameworks;
    ...
    for (let i = 0; i < runFrameworks.length; i++) {
        ...
        try {
            ...
            let str = init((_a = framework.shadowRootName) !== null && _a !== void 0 ? _a : "");
            ...
            console.log(framework.fullNameWithKeyedAndVersion +
                " is " +
