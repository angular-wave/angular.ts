import { Capabilities, Condition } from "selenium-webdriver";
...
export async function findById(driver, id, isInButtonArea) {
    let root = mainRoot(driver, isInButtonArea);
    if (config.LOG_DEBUG)
        console.log("findById selector", `${root}.querySelector('#${id}')`);
    return await driver.executeScript(`return ${root}.querySelector('#${id}')`);
}
...
export async function testTextContains(driver, xpath, text, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testTextContains ${xpath} ${text}`, async function (driver) {
        try {
            if (config.LOG_DEBUG)
                console.log("testTextContains", xpath);
...
export async function testTextNotContained(driver, xpath, text, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testTextNotContained ${xpath} ${text}`, async function (driver) {
        try {
            let elem = await findByXPath(driver, xpath, isInButtonArea);
            if (elem == null)
                return false;
...
export async function testClassContains(driver, xpath, text, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testClassContains ${xpath} ${text}`, async function (driver) {
        try {
            let elem = await findByXPath(driver, xpath, isInButtonArea);
            if (elem == null)
                return false;
...
export async function testElementLocatedByXpath(driver, xpath, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testElementLocatedByXpath ${xpath}`, async function (driver) {
        try {
            let elem = await findByXPath(driver, xpath, isInButtonArea);
...
export async function testElementNotLocatedByXPath(driver, xpath, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testElementNotLocatedByXPath ${xpath}`, async function (driver) {
        try {
            let elem = await findByXPath(driver, xpath, isInButtonArea);
            if (config.LOG_DEBUG)
...
export async function testElementLocatedById(driver, id, timeout = config.TIMEOUT, isInButtonArea) {
    return await waitForCondition(driver)(`testElementLocatedById ${id}`, async function (driver) {
        try {
            let root = mainRoot(driver, isInButtonArea);
            if (config.LOG_DEBUG)
...
export async function retry(retryCount, driver, fun) {
    let lastError;
...
export async function clickElementById(driver, id, isInButtonArea) {
    return await retry(5, driver, async function (driver) {
        let elem = await findById(driver, id, isInButtonArea);
        if (config.LOG_DEBUG)
            console.log("clickElementById:", elem);
        if (!elem)
            throw new Error(`Element with id ${id} not found`);
        await elem.click();
    });
}
export async function clickElementByXPath(driver, xpath, isInButtonArea) {
...
export function mainRoot(driver, isInButtonArea) {
    if (useShadowRoot) {
