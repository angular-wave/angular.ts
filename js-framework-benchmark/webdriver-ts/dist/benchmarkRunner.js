...
import { BenchmarkRunner, config, initializeFrameworks, } from "./common.js";
import { fork } from "node:child_process";
import * as fs from "node:fs";
import { performance } from "node:perf_hooks";
import { benchmarkInfos, BenchmarkType, cpuBenchmarkInfosArray, } from "./benchmarksCommon.js";
import { writeResults } from "./writeResults.js";
import { PlausibilityCheck } from "./timeline.js";
function forkAndCallBenchmark(framework, benchmarkInfo, benchmarkOptions) {
...
async function runBenchmakLoopSize(framework, benchmarkInfo, benchmarkOptions) {
    let warnings = [];
    let errors = [];
    let results = [];
...
async function runBenchmakLoop(framework, benchmarkInfo, benchmarkOptions, plausibilityCheck) {
    let warnings = [];
    let errors = [];
    let results = [];
    let count = 0;
...
async function runBench(runFrameworks, benchmarkInfos, benchmarkOptions) {
    let errors = [];
    let warnings = [];
    ...
    console.log("Benchmarks that will be run", benchmarkInfos.map((b) => b.id));
        ...
        for (let i = 0; i < runFrameworks.length; i++) {
            try {
                ...
                if (benchmarkInfos[j].type == BenchmarkType.SIZE_MAIN) {
                    result = await runBenchmakLoopSize(runFrameworks[i], benchmarkInfos[j], benchmarkOptions);
                }
                else if (benchmarkInfos[j].type == BenchmarkType.CPU) {
                    result = await runBenchmakLoop(runFrameworks[i], benchmarkInfos[j], benchmarkOptions, plausibilityCheck);
                }
                else {
                    result = await runBenchmakLoop(runFrameworks[i], benchmarkInfos[j], benchmarkOptions, plausibilityCheck);
                }
...
async function main() {
    ...
    let args = yargs(process.argv)
        .usage("$0 [--framework Framework1 Framework2 ...] [--benchmark Benchmark1 Benchmark2 ...] [--chromeBinary path] \n or: $0 [directory1] [directory2] .. [directory3]")
        .help("help")
        .boolean("headless")
    ...
    let benchmarkOptions = {
        ...
        puppeteerSleep: (_a = args.puppeteerSleep) !== null && _a !== void 0 ? _a : 0,
    };
    config.PUPPETEER_WAIT_MS = (_b = benchmarkOptions.puppeteerSleep) !== null && _b !== void 0 ? _b : 0;
    if (args.count) {
    ...
    let runBenchmarksArgs = args.benchmark && args.benchmark.length > 0 ? args.benchmark : [""];
    let runBenchmarks = benchmarkInfos.filter((b) => 
    ...
    let runFrameworks;
    let matchesDirectoryArg = (directoryName) => frameworkArgument.length === 0 || frameworkArgument.some((arg) => arg == directoryName);
