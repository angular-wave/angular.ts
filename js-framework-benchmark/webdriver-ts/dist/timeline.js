 ...
 * @returns
 */
export function extractRelevantEvents(entries, startLogicEvent) {
    let filteredEvents = [];
        ...
        if (e.name === "EventDispatch") {
            if (e.args.data.type === startLogicEvent) {
                if (config.LOG_DETAILS)
                    console.log("startLogicEvent", e.args.data.type, +e.ts);
            ...
            if (config.LOG_DETAILS)
                console.log("Layout", +e.ts, +e.ts + e.dur - startLogicEvent_startTS);
            ...
            if (config.LOG_DETAILS)
                console.log("PAINT", +e.ts, +e.ts + e.dur - startLogicEvent_startTS);
...
async function fetchEventsFromPerformanceLog(fileName, startLogicEventName) {
    let timingResults = [];
    let entries = [];
    do {
...
export async function computeResultsCPU(fileName, startLogicEventName = "click") {
...
export class PlausibilityCheck {
    ...
    check(result, trace, framework, benchmarkInfo) {
        ...
        let val = (_a = this.maxDeltaBetweenCommits.get(framework.fullNameWithKeyedAndVersion)) !== null && _a !== void 0 ? _a : 0;
        ...
        val = (_b = this.raf_long_delays.get(framework.fullNameWithKeyedAndVersion)) !== null && _b !== void 0 ? _b : 0;
...
export async function parseCPUTrace(benchmarkOptions, framework, benchmarkInfo, plausibilityCheck, startLogicEventName) {
    let results = [];
